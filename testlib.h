...
class random_t {
   ...
   public:
    ...
    unsigned long long next(unsigned long long from, unsigned long long to) {
        if (from > to)
            __testlib_fail(
                "random_t::next(unsigned long long from, unsigned long long "
    ...
    typename Iter::value_type any(const Iter &begin, const Iter &end) {
        ...
        if (size <= 0)
            __testlib_fail(
                "random_t::any(const Iter& begin, const Iter& end): range must "
    ...
    long long wnext(long long from, long long to, int type) {
        if (from > to)
            __testlib_fail(
                "random_t::wnext(long long from, long long to, int type): from "
                "can't not exceed to");
        return wnext(to - from + 1, type) + from;
    }
    ...
    typename Iter::value_type wany(const Iter &begin, const Iter &end,
        ...
        if (size <= 0)
            __testlib_fail(
                "random_t::any(const Iter& begin, const Iter& end, int type): "
    ...
    std::vector<T> partition(int size, T sum, T min_part) {
        ...
        if (*std::min_element(result.begin(), result.end()) < min_part)
            __testlib_fail(
                "random_t::partition: partition min is expected to be no less "
...
static inline double stringToStrictDouble(InStream &in, const char *buffer,
    ...
    if (minAfterPointDigitCount > maxAfterPointDigitCount)
        in.quit(_fail,
                "stringToStrictDouble: minAfterPointDigitCount should be less "
    ...
    std::string testset() const {
        if (!_initialized)
            __testlib_fail(
                ...
                "argv) instead of registerTestlibCmd() to support "
                "checker.testset()");
        return _testset;
    }
