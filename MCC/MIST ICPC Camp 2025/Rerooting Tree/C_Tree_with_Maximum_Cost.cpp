...
void solve() {
    ...
    auto merge_into = [](Aggregate vertex_dp, Value neighbor_dp, int vertex,
        ...
        ll mainCost = vertex_dp.first + neighbor_dp.first;
        ll cost = vertex_dp.second + neighbor_dp.second;
        return {mainCost, cost};
    };
    auto finalize_merge = [&](Aggregate vertex_dp, int vertex,
                              int edge_index) -> Value {
        ll mainCost = vertex_dp.first + vertex_dp.second;
        ll cost = vertex_dp.second + val[vertex];
        return {mainCost, cost};
    };
