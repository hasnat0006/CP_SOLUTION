...
inline void solve() {
    ...
    map<unsigned long long, bool> cal;
    ...
    for (int i = 0; i < n; i++) {
        ...
        bitset<31> b = a[i];
        b.flip();
        unsigned long long s = (b.to_ulong());
        if (freq[s]) {
            cal[s] = 1;
        }
        int original = freq[a[i]], rev = freq[s];
        int pair = min(original, rev);
        g += pair;
        int extra = max(original, rev) - pair;
        g += extra;
        cal[a[i]] = 1;
    }
